import 'package:drift/drift.dart' show Value;
import 'package:flutter_test/flutter_test.dart';

import 'package:pos_app/core/database/app_database.dart';
import 'package:pos_app/core/services/audit_service.dart';
import 'package:pos_app/features/cart/domain/cart_item.dart';
import 'package:pos_app/features/cart/presentation/providers/cart_notifier.dart';
import 'package:pos_app/features/cart/data/place_order.dart';
import 'package:pos_app/features/returns/domain/process_return.dart';

import '../../_support/test_db.dart';

void main() {
  late AppDatabase db;
  late AuditService audit;

  setUp(() {
    db = openInMemoryDatabase();
    audit = AuditService(db);
  });

  tearDown(() async {
    await db.close();
  });

  Future<int> seedProduct({
    required String sku,
    required String name,
    int stock = 10,
    bool composite = false,
  }) =>
      db.productsDao.upsert(
        ProductsCompanion.insert(
          sku: sku,
          name: name,
          price: 5.0,
          stockQuantity: Value(stock),
          isComposite: Value(composite),
        ),
      );

  Future<int> placeSimpleOrder(int productId, {int qty = 2}) => placeOrder(
        db,
        audit,
        cart: [
          CartItem(
            productId: productId,
            name: 'Item',
            unitPrice: 5.0,
            quantity: qty,
            isTaxable: false,
          ),
        ],
        summary: CartSummary(
          subtotal: 5.0 * qty,
          orderDiscount: 0,
          loyaltyDiscount: 0,
          taxLines: const [],
          taxAmount: 0,
          total: 5.0 * qty,
          taxEnabled: false,
          pointsToEarn: 0,
        ),
        session: CartSession(
          ticketNumber: '#0001',
          openedAt: DateTime(2026, 1, 1),
          customerId: null,
          loyaltyPointsToRedeem: 0,
        ),
        paymentMethod: 'cash',
        tenderedAmount: 5.0 * qty,
      );

  test('refund with restock returns inventory and writes audit log', () async {
    final productId = await seedProduct(sku: 'COFFEE', name: 'Coffee', stock: 10);
    final orderId = await placeSimpleOrder(productId, qty: 3);

    // Stock should be 7 after sale of 3
    expect((await db.productsDao.getById(productId))!.stockQuantity, 7);

    final returnId = await processReturn(
      db,
      audit,
      orderId: orderId,
      amount: 15.0,
      restock: true,
      reason: 'damaged',
    );

    expect(returnId, greaterThan(0));

    // Stock back to 10
    expect((await db.productsDao.getById(productId))!.stockQuantity, 10);

    // Order flipped to refunded
    expect((await db.ordersDao.getById(orderId))!.status, 'refunded');

    // Returns row inserted
    final returns = await db.select(db.returns).get();
    expect(returns, hasLength(1));
    expect(returns.first.amount, 15.0);
    expect(returns.first.restock, true);
    expect(returns.first.reason, 'damaged');

    // Stock adjustment with reason 'return' present
    final adjustments =
        await db.inventoryDao.getAdjustmentsForProduct(productId);
    expect(adjustments, hasLength(2),
        reason: 'one for the original sale, one for the refund');
    final returnAdj = adjustments.firstWhere((a) => a.reasonCode == 'return');
    expect(returnAdj.delta, 3);

    // Audit log: place + refund
    final auditRows = await db.select(db.auditLog).get();
    expect(auditRows, hasLength(2));
    expect(auditRows.last.action, 'refund');
    expect(auditRows.last.entityId, orderId);
  });

  test('refund without restock leaves inventory alone', () async {
    final productId = await seedProduct(sku: 'COFFEE', name: 'Coffee', stock: 10);
    final orderId = await placeSimpleOrder(productId, qty: 3);

    expect((await db.productsDao.getById(productId))!.stockQuantity, 7);

    await processReturn(
      db,
      audit,
      orderId: orderId,
      amount: 5.0,
      restock: false,
    );

    // Stock unchanged
    expect((await db.productsDao.getById(productId))!.stockQuantity, 7);

    // Only the original sale adjustment, no return adjustment
    final adjustments =
        await db.inventoryDao.getAdjustmentsForProduct(productId);
    expect(adjustments, hasLength(1));
    expect(adjustments.first.reasonCode, 'sale');

    // Order still flipped to refunded
    expect((await db.ordersDao.getById(orderId))!.status, 'refunded');
  });

  test('refund of composite order restocks components, not the composite',
      () async {
    final espressoId = await seedProduct(sku: 'ESP', name: 'Espresso', stock: 100);
    final milkId = await seedProduct(sku: 'MLK', name: 'Milk', stock: 50);
    final latteId =
        await seedProduct(sku: 'LAT', name: 'Latte', stock: 0, composite: true);

    await db.productsDao.replaceComponents(latteId, [
      ProductComponentsCompanion.insert(
        compositeProductId: latteId,
        componentProductId: espressoId,
      ),
      ProductComponentsCompanion.insert(
        compositeProductId: latteId,
        componentProductId: milkId,
      ),
    ]);

    final orderId = await placeOrder(
      db,
      audit,
      cart: [
        CartItem(
          productId: latteId,
          name: 'Latte',
          unitPrice: 5.0,
          quantity: 4,
          isTaxable: false,
        ),
      ],
      summary: const CartSummary(
        subtotal: 20.0,
        orderDiscount: 0,
        loyaltyDiscount: 0,
        taxLines: [],
        taxAmount: 0,
        total: 20.0,
        taxEnabled: false,
        pointsToEarn: 0,
      ),
      session: CartSession(
        ticketNumber: '#0001',
        openedAt: DateTime(2026, 1, 1),
        customerId: null,
        loyaltyPointsToRedeem: 0,
      ),
      paymentMethod: 'cash',
      tenderedAmount: 20.0,
    );

    // After sale: composite untouched, components down by 4 each
    expect((await db.productsDao.getById(latteId))!.stockQuantity, 0);
    expect((await db.productsDao.getById(espressoId))!.stockQuantity, 96);
    expect((await db.productsDao.getById(milkId))!.stockQuantity, 46);

    await processReturn(
      db,
      audit,
      orderId: orderId,
      amount: 20.0,
      restock: true,
    );

    // Components fully restocked, composite still untouched
    expect((await db.productsDao.getById(latteId))!.stockQuantity, 0);
    expect((await db.productsDao.getById(espressoId))!.stockQuantity, 100);
    expect((await db.productsDao.getById(milkId))!.stockQuantity, 50);
  });

  test('refund on missing order throws and persists nothing', () async {
    expect(
      () => processReturn(
        db,
        audit,
        orderId: 9999,
        amount: 5.0,
        restock: true,
      ),
      throwsStateError,
    );

    expect(await db.select(db.returns).get(), isEmpty);
    expect(await db.select(db.auditLog).get(), isEmpty);
  });
}
