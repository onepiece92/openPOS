...
function parseVerificationItems(content) {
    const items = [];
    const gapSection = /## gaps?|## issues?|## failures?/i;
    let inGapSection = false;
    for (const line of content.split('\n')) {
        if (/^##/.test(line)) {
            inGapSection = gapSection.test(line);
            continue;
        }
        if (inGapSection && line.trim().startsWith('-'))
            items.push(line.trim());
    }
    return items;
}
...
export const auditUat = async (_args, projectDir) => {
