 ...
 */
export const verifyPhaseCompleteness = async (args, projectDir) => {
    const phase = args[0];
    if (!phase) {
...
// ─── verifySchemaDrift ────────────────────────────────────────────────────
export const verifySchemaDrift = async (args, projectDir) => {
    ...
    try {
        ...
        for (const entry of entries) {
            if (!entry.isDirectory())
                continue;
            if (phaseArg && !entry.name.startsWith(normalizePhaseName(phaseArg)))
                continue;
