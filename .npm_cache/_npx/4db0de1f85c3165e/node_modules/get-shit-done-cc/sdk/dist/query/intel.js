...
// ─── Handlers ────────────────────────────────────────────────────────────
export const intelStatus = async (_args, projectDir) => {
...
export const intelDiff = async (_args, projectDir) => {
    if (!isIntelEnabled(projectDir)) {
...
export const intelSnapshot = async (_args, projectDir) => {
    if (!isIntelEnabled(projectDir)) {
        return { data: { disabled: true, message: 'Intel system disabled.' } };
    }
...
export const intelValidate = async (_args, projectDir) => {
    if (!isIntelEnabled(projectDir)) {
        return { data: { disabled: true, message: 'Intel system disabled.' } };
    }
    const errors = [];
...
export const intelQuery = async (args, projectDir) => {
    const term = args[0] || '';
    if (!isIntelEnabled(projectDir)) {
        return { data: { disabled: true, message: 'Intel system disabled.' } };
    }
...
export const intelExtractExports = async (args, projectDir) => {
    ...
    if (esmExports.length > 0 && exports.length > esmExports.length)
        method = 'mixed';
    else if (esmExports.length > 0 && method === 'none')
        method = 'esm';
    return { data: { file: args[0], exports, method } };
};
export const intelPatchMeta = async (args, projectDir) => {
    const filePath = args[0] ? resolve(projectDir, args[0]) : '';
    if (!filePath || !existsSync(filePath)) {
