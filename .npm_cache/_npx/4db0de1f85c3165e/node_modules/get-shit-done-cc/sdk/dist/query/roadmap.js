 ...
 */
export async function getMilestoneInfo(projectDir) {
    try {
 ...
 */
export async function extractCurrentMilestone(content, projectDir) {
    ...
    const nextMilestonePattern = new RegExp(`^#{1,${headingLevel}}\\s+(?:.*v\\d+\\.\\d+|✅|📋|🚧)`, 'mi');
 ...
 */
export const roadmapGetPhase = async (args, projectDir) => {
 ...
 */
export const roadmapAnalyze = async (_args, projectDir) => {
    const roadmapPath = planningPaths(projectDir).roadmap;
    let rawContent;
    try {
        rawContent = await readFile(roadmapPath, 'utf-8');
...
export const roadmapUpdatePlanProgress = async (args, projectDir) => {
    const phase = args[0];
    ...
    try {
        ...
        const updated = content.replace(/(-\s*\[\s*\]\s*(?:Plan\s+\d+|plan\s+\d+|\*\*Plan))/gi, (match) => match.replace('[ ]', '[x]'));
        if (updated !== content) {
...
export const requirementsMarkComplete = async (args, projectDir) => {
    const reqIds = args;
    const paths = planningPaths(projectDir);
    if (reqIds.length === 0) {
    ...
    try {
        ...
        for (const id of reqIds) {
            ...
            if (content.includes(`[x]`) && content.includes(id))
                changeCount++;
        }
