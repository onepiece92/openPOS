...
import { spliceFrontmatter } from './frontmatter-mutation.js';
 ...
 */
export const templateSelect = async (args, projectDir) => {
 ...
 */
export const templateFill = async (args, projectDir) => {
    const templateType = args[0];
    const outputPath = args[1];
    if (!templateType) {
        throw new GSDError('template type required: summary, plan, or verification', ErrorClassification.Validation);
    }
    if (!outputPath) {
        throw new GSDError('output path required', ErrorClassification.Validation);
    }
    ...
    if (rel.startsWith('..') || rel.includes('..')) {
        throw new GSDError(`Output path escapes project directory: ${outputPath}`, ErrorClassification.Validation);
    }
    ...
    switch (templateType) {
        ...
        default:
            throw new GSDError(`Unknown template type: ${templateType}. Available: summary, plan, verification`, ErrorClassification.Validation);
    }
    // Apply overrides
