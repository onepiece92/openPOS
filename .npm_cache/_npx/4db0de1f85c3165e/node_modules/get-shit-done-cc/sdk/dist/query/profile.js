 ...
 */
export const learningsQuery = async (args) => {
    ...
    if (tag) {
        results = results.filter(r => Array.isArray(r.tags) && r.tags.includes(tag));
    }
    if (limit && limit > 0) {
...
export const learningsCopy = async (_args, projectDir) => {
...
export const scanSessions = async (_args, _projectDir) => {
...
export const profileSample = async (_args, _projectDir) => {
    if (!existsSync(SESSIONS_DIR)) {
...
export const profileQuestionnaire = async (args, _projectDir) => {
    const answersFlag = args.indexOf('--answers');
    if (answersFlag >= 0 && args[answersFlag + 1]) {
        try {
            const answers = JSON.parse(readFileSync(resolve(args[answersFlag + 1]), 'utf-8'));
            const analysis = {};
...
export const writeProfile = async (args, projectDir) => {
    ...
    try {
        const analysis = JSON.parse(readFileSync(resolve(inputPath), 'utf-8'));
...
export const generateClaudeProfile = async (args, _projectDir) => {
...
export const generateClaudeMd = async (_args, projectDir) => {
    const safeRead = (path) => {
        try {
